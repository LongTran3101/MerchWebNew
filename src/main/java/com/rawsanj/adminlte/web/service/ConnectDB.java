...
public class ConnectDB {
	...
	public String getAdress(String key) {
		String adr = "";
		try {
			String sql = "select address from tbl_info where id_key =" + key
					+ " and status = 1 and Ngay_tao <= ENDDATE";
	...
	public Boolean insert(SaleMerch merch) {
		...
		try {
			try {
				...
				java.util.Date date = new java.util.Date();
				java.sql.Date sqlDate = new java.sql.Date(date.getTime());
				String sql = "INSERT INTO `sale_merch`( `name`, `ip`, `day`, `sale`, `money`, `email`) VALUES (?,?,?,?,?,?) ";
				PreparedStatement ps = con.prepareStatement(sql);
				ps.setString(1, merch.getName());
				ps.setString(2, merch.getIp());
				ps.setDate(3, new java.sql.Date(df.parse(merch.getDayString()).getTime()));
	...
	public Boolean insertAccountMerch(AccountMerch merch) {
		...
		try {
			try {
				...
				java.util.Date date = new java.util.Date();
				java.sql.Date sqlDate = new java.sql.Date(date.getTime());
				String sql = "INSERT INTO `account_merch`( `name`, `ip`, `email`,`path`,`day`,`sale`,`money`) VALUES (?,?,?,?,?,0,0) ";
				PreparedStatement ps = con.prepareStatement(sql);
				ps.setString(1, merch.getName());
				ps.setString(2, merch.getIp());
				ps.setString(3, merch.getEmail());
				ps.setString(4, merch.getPath());
	...
	public boolean deleteAccountMerch(int id) {
		try {
			try {
				DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
				java.util.Date date = new java.util.Date();
				java.sql.Date sqlDate = new java.sql.Date(date.getTime());
				String sql = "DELETE FROM `account_merch` WHERE `id`= ?";
				PreparedStatement ps = con.prepareStatement(sql);
				ps.setInt(1,id);
				int row = 0;
				row = ps.executeUpdate();
	...
	public List<SaleMerch> getSaleByDay(String daySeach) {
		try {
			try {
				DateFormat df = new SimpleDateFormat("yyyy-MM-dd");

				List<SaleMerch> lst = new ArrayList<>();
				String sql = "select * from sale_merch where  ? = day   ";
				statement = con.prepareStatement(sql);
	...
	// for (Image entity : a) {
	// String sql = "INSERT INTO `spyhours` (`id`, `value`, `count`,
	// `time`,`contry`) VALUES (NULL, '"+entity.value+"', '"+entity.count+"',
	// '"+date+" 00:00:00','"+contry+"')";
	// try {
	// st.execute(sql);
	...
	//
	// String sql = "INSERT INTO `spyhours` (`id`, `value`, `count`,
	// `time`,`contry`) VALUES (NULL, '1', '1', '"+date+" 00:00:00','1')";
	// try {
	// st.execute(sql);
	// } catch (SQLException e) {
	// // TODO Auto-generated catch block
